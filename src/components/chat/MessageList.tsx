...
function toolLabel(toolName: string, args: any): string {
  ...
  if (toolName === "str_replace_editor") {
    switch (args?.command) {
      case "create":     return `Created ${path}`;
      case "str_replace":
      case "insert":     return `Edited ${path}`;
      case "view":       return `Viewed ${path}`;
      default:           return `Updated ${path}`;
    }
  }
  if (toolName === "file_manager") {
    switch (args?.command) {
      case "rename": return `Renamed ${path}`;
      case "delete": return `Deleted ${path}`;
    }
  }
  return toolName;
}
                              ...
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-neutral-50 rounded-lg text-xs border border-neutral-200">
                                {tool.state === "result" && tool.result ? (
                                  <>
                                    ...
                                    <span className="text-neutral-700">{toolLabel(tool.toolName, tool.args)}</span>
                                  </>
                                ) : (
                                  <>
